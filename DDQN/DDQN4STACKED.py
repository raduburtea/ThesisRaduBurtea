...
def create_cnn_vectorization():
    ...
    model.add(Conv2D(filters = 16, kernel_size = 8, strides = (4,4), input_shape=( 84, 84, 1)))
    model.add(Activation('relu'))
    model.add(Conv2D(filters = 32, kernel_size = 4,  input_shape=( 84, 84, 1)))
    model.add(Activation('relu'))
    model.add(Flatten())
...
class Model:
    ...
    def target_train(self):
        ...
        for i in range(len(target_weights)):
            target_weights[i] = target_weights[i]*0.1 + 0.9*weights[i]
...
class ImageMemory:
    def __init__(self):
        self.images = [np.zeros((84,84,1)) for i in range(4)]

    def add_image(self, image):

            self.images.pop(0)
            self.images.append(image.reshape(84,84,1))
...
def play(agent,  epsilon):
    ...
    while not done:
        env.render()
        intermed = rgb2gray(observation).reshape(-1, 84, 84, 1)
        images.add_image(intermed)
        state = images.get_stacked_images()
        ...
        intermed = rgb2gray(observation).reshape(-1, 84, 84, 1)
        images.add_image(intermed)
        new_state = images.get_stacked_images()
...
with open('runstacked.txt', 'w') as filehandle:
            for listitem in totalrewards:
                filehandle.write('%s\n' % str(listitem))

env.close()
plt.plot(totalrewards)
plt.savefig('runstacked.png')
plt.show()
plt.close()
# plt.plot(totalrewards)
# plt.show()
